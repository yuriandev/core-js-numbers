...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarTwoVectors = x1 * x2 + y1 * y2;
  const value1 = Math.sqrt(x1 * x1 + y1 * y1);
  const value2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(scalarTwoVectors / (value1 * value2));
}
...
function getFloatOnString(str) {
  return Number.isNaN(parseFloat(str)) ? NaN : Number.parseFloat(str);
}
